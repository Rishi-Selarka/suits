...
export default function TrapDetectorPage({ result }: { result: AnalysisResult | null }) {
  const traps = result ? findTraps(result) : []

  return (
    <ToolLayout title="Trap Clause Detector" description="Hidden risks, one-sided terms, and deceptive language" icon={Eye} result={result}>
      {traps.length === 0 ? (
